...
class _ResultScreenState extends State<ResultScreen> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () => setState(() => _loading = false));
  }

  fetchUpdateUserDetails() async {
      ...
      () {
        ...
        if (_isWinner) {
          ...
          context.read<UpdateScoreAndCoinsCubit>().updateCoinsAndScore(
                ...
                currentUser.points,
                true,
            (earnedCoins * 0.9).toInt(),
                wonBattleKey,
              );
          //update score locally and database
          context.read<UserDetailsCubit>().updateCoins(
                addCoin: true,
                coins: (earnedCoins * 0.9).toInt(),
              );
          context.read<UpdateScoreAndCoinsCubit>().updateAdminCoins(
            "1",
            (earnedCoins * 0.1).toInt(),
            true,
            "commission ",
          );
                      ...
                      ? context.read<UserDetailsCubit>().userId() == _winnerId
                          ? Padding(
                              ...
                              child: Container(
                                ...
                                child: Text(
                                  "${AppLocalization.of(context)!.getTranslatedValues("youWin")!} ${((widget.entryFee! * 2) * 0.9).toInt()} ${AppLocalization.of(context)!.getTranslatedValues("coinsLbl")!}",
                                  style: TextStyle(
                                      fontSize: 20,
  ...
  Widget _buildReviewAnswersButton() {
    void onTapYesReviewAnswers() {
      ...
      context.read<UpdateScoreAndCoinsCubit>().updateCoins(
          context.read<UserDetailsCubit>().userId(),
          (reviewAnswersDeductCoins * 0.9).toInt(),
          false,
          reviewAnswerLbl);
      context.read<UserDetailsCubit>().updateCoins(
            addCoin: false,
        coins: (reviewAnswersDeductCoins * 0.9).toInt(),       );
      ...
      () {
        ...
        showDialog(
          ...
          builder: (_) => AlertDialog(
            ...
            content: Text(
              "${context.read<SystemConfigCubit>().reviewAnswersDeductCoins} ${AppLocalization.of(context)!.getTranslatedValues(coinsWillBeDeductedKey)!}",
