  ...
  void battleRoomListener(
    ...
    Future.delayed(Duration.zero, () async {
      if (await InternetConnectivity.isUserOffline()) {
        heLostConnection = true;
        int i = (context.read<BattleRoomCubit>().getEntryFee() * 0.95).toInt();
        showDialog<bool>(
          context: context,
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                            builder: (context) => HomeScreen(
                                isGuest: false,
                                giveConnexionFees: true,
                                entryFees: i
                            )),
                      );
                    }
                  },
                  child: Text("Get back entryFees",
                    style: TextStyle(
        ...
        if (state is BattleRoomUserFound) {
          ...
          if (context
            ...
            return _buildYouWonContainer(() {
              ...
              context.read<UpdateScoreAndCoinsCubit>().updateCoins(
                _currUserId,
                (context.read<BattleRoomCubit>().getEntryFee() * 1.8).toInt(),
                true,
                wonBattleKey,
              );
              context.read<UpdateScoreAndCoinsCubit>().updateAdminCoins(
                "1",
                (context.read<BattleRoomCubit>().getEntryFee() * 0.2).toInt(),
                true,
                "commission",
              );
              context.read<UserDetailsCubit>().updateCoins(
            ...
            children: [
              Align(
                ...
                child: QuestionsContainer(
                  ...
                  context.read<SystemConfigCubit>().showAnswerCorrectness,
                  lifeLines: const {},
